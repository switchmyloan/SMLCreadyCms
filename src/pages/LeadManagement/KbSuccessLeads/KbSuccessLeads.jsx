...
// KB message buckets — match the backend's ILIKE patterns. Used both for
// the funnel cards and the "Outcome" pill in the table so the two stay in
// sync.
const classifyMessage = (msg) => {
  if (!msg) return 'other';
  const m = String(msg);
  if (/Duplicate Pan/i.test(m)) return 'duplicate';
  if (/Lead status\s*:\s*Approved/i.test(m)) return 'approved';
  if (/Rejected/i.test(m)) return 'rejected';
  return 'other';
};

const BUCKETS = [
  { key: 'approved',  label: 'Approved',     field: 'approved',  icon: BadgeCheck,    color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-200', ring: 'ring-emerald-400', barColor: 'bg-emerald-500', iconBg: 'bg-emerald-100' },
  { key: 'rejected',  label: 'Rejected',     field: 'rejected',  icon: XCircle,       color: 'text-rose-600',    bg: 'bg-rose-50',    border: 'border-rose-200',    ring: 'ring-rose-400',    barColor: 'bg-rose-500',    iconBg: 'bg-rose-100' },
  { key: 'duplicate', label: 'Duplicate PAN',field: 'duplicate', icon: AlertTriangle, color: 'text-amber-600',   bg: 'bg-amber-50',   border: 'border-amber-200',   ring: 'ring-amber-400',   barColor: 'bg-amber-500',   iconBg: 'bg-amber-100' },
  { key: 'other',     label: 'Other',        field: 'other',     icon: HelpCircle,    color: 'text-gray-600',    bg: 'bg-gray-50',    border: 'border-gray-200',    ring: 'ring-gray-400',    barColor: 'bg-gray-500',    iconBg: 'bg-gray-100' },
];

const FunnelCards = ({ funnel, activeBucket, onBucketClick, loading }) => {
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
      <button
        ...
        className={`group text-left p-4 bg-white rounded-xl border transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 ${
          activeBucket === '' ? 'ring-2 ring-gray-300 border-gray-200 shadow-md' : 'border-gray-100 shadow-sm'
        }`}
      >
      ...
      {BUCKETS.map((s) => {
        ...
        return (
          <button
            key={s.key}
            type="button"
            onClick={() => onBucketClick(isActive ? '' : s.key)}
            className={`group text-left p-4 bg-white rounded-xl border transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 ${
...
const outcomeVariant = (bucket) => {
  if (bucket === 'approved') return 'success';
  if (bucket === 'rejected') return 'danger';
  if (bucket === 'duplicate') return 'warning';
  return 'default';
};

const outcomeLabel = (bucket) => {
  if (bucket === 'approved') return 'Approved';
  if (bucket === 'rejected') return 'Rejected';
  if (bucket === 'duplicate') return 'Duplicate PAN';
  return 'Other';
};

const KbSuccessLeads = () => {
  const [rawData, setRawData] = useState([]);
  const [totalDataCount, setTotalDataCount] = useState(0);
  const [loading, setLoading] = useState(false);
  const [funnel, setFunnel] = useState({
    total: 0, approved: 0, rejected: 0, duplicate: 0, other: 0, todaySubmitted: 0,
  });
  ...
  // Guard against React StrictMode double-firing the mount effect — the
  // backend funnel query is a GROUP BY across all KB lender_responses;
  // we don't want two of them on a dev reload.
  ...
  const fetchLeads = useCallback(async () => {
    try {
      ...
      if (query.bucket) filters.bucket = query.bucket;

      const response = await getKbSuccessLeads(query.page_no, query.limit, filters);
      if (response?.data?.success) {
        ...
        setRawData([]);
        setTotalDataCount(0);
        ToastNotification.error('Failed to fetch KB leads');
      }
    } catch (err) {
      console.error(err);
    ...
    query.page_no, query.limit, query.filter_date, query.search, query.gender,
    query.minIncome, query.maxIncome, query.startDate, query.endDate, query.bucket,
  ]);

  useEffect(() => {
    if (initialFetchRef.current === false) {
  ...
  const handleExport = useCallback(async () => {
    try {
      ...
      const response = await getAllKbSuccessLeads(filters);
      ...
      a.href = url; a.download = `kb-success-leads-${new Date().toISOString().slice(0, 10)}.csv`; a.click();
  ...
  const hasActiveFilters =
    query.search || query.gender || query.bucket || query.filter_date ||
    query.startDate || (query.minIncome !== undefined);

  const clearAllFilters = () => {
    setQuery({
      page_no: 1, limit: 10, search: '', filter_date: '', startDate: null, endDate: null,
      gender: '', minIncome: undefined, maxIncome: undefined, bucket: '',
    });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold text-gray-900">KB Success Leads</h2>
          <p className="text-sm text-gray-500 mt-0.5">Outcome breakdown of leads submitted to KreditBee, grouped by lender response message.</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 bg-indigo-50 border border-indigo-200 rounded-lg">
            ...
            <span className="text-[11px] font-medium text-indigo-700">Partner: KreditBee</span>
          </div>
      ...
      <FunnelCards funnel={funnel} activeBucket={query.bucket} onBucketClick={(bucket) => updateQuery({ bucket })} loading={loading && !rawData.length} />

      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-100">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-semibold text-gray-900">KB Leads</h3>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <select
                value={query.bucket}
                onChange={(e) => updateQuery({ bucket: e.target.value })}
                className="px-2.5 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 bg-white font-medium text-gray-600"
              >
                <option value="">All Outcomes</option>
                <option value="approved">Approved</option>
                <option value="rejected">Rejected</option>
                <option value="duplicate">Duplicate PAN</option>
                <option value="other">Other</option>
              </select>
              ...
              <div className="relative">
                <input
                  type="text"
                  placeholder="Search name, phone, PAN, message..."
                  value={query.search}
                  onChange={(e) => updateQuery({ search: e.target.value })}
                  className="w-56 pl-7 pr-3 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 transition"
                />
        ...
        <style>{`
          .kb-table-scroll::-webkit-scrollbar { height: 6px; }
          .kb-table-scroll::-webkit-scrollbar-track { background: #f9fafb; }
          .kb-table-scroll::-webkit-scrollbar-thumb { background: #d1d5db; border-radius: 3px; }
          .kb-table-scroll::-webkit-scrollbar-thumb:hover { background: #9ca3af; }
        `}</style>
        ...
        {loading && !rawData.length ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-gray-400 text-sm mt-3">Loading KB leads...</p>
          </div>
        ) : rawData.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-sm font-medium text-gray-500">No KB leads found</p>
            {hasActiveFilters && (
          ...
          <div className="kb-table-scroll overflow-x-auto">
            <table className="w-full min-w-[1100px]">
              <thead>
                <tr className="bg-gray-50/80">
                  ...
                  <th className="px-4 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-500">PAN</th>
                  <th className="px-4 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-500">Income</th>
                  <th className="px-4 py-2.5 text-center text-[11px] font-semibold uppercase tracking-wider text-gray-500">Outcome</th>
                  <th className="px-4 py-2.5 text-center text-[11px] font-semibold uppercase tracking-wider text-gray-500">Status</th>
                  <th className="px-4 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-500 min-w-[260px]">Message</th>
                  <th className="px-4 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-500">Partner Lead Id</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {rawData.map((row, index) => {
                  ...
                  const bucket = classifyMessage(row.message);
                  const partnerLeadId = row?.metadata?.requestBody?.partnerLeadId;
                  return (
                    <tr key={row.id || index} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-xs font-mono text-gray-600">{lead.panNumber || <span className="text-gray-300">-</span>}</td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <Badge variant={outcomeVariant(bucket)}>{outcomeLabel(bucket)}</Badge>
                      </td>

                      <td className="px-4 py-3 text-center">{row.status ? <Badge variant="default">{row.status}</Badge> : <span className="text-gray-300">-</span>}</td>

                      <td className="px-4 py-3">
                        {row.message ? (
                          <div className="text-xs text-gray-600 truncate max-w-[260px]" title={row.message}>{row.message}</div>
                        ) : <span className="text-gray-300">-</span>}
                      </td>

                      <td className="px-4 py-3 text-xs font-mono text-gray-500">
                        {partnerLeadId ? (
                          <span className="truncate max-w-[160px] inline-block" title={partnerLeadId}>{partnerLeadId}</span>
                        ) : <span className="text-gray-300">-</span>}
                      </td>
