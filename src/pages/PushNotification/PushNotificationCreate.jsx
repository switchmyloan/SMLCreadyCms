...
export default function PushNotificationCreate() {
  ...
  // Columns for the Push Notification History table
  const historyColumns = [
    ...
    },
    {
      ...
      header: "Audience (Chips)",
      cell: ({ row }) => row.original.audience.join(", ")
    },
    {
      accessorKey: "selectedUserIds",
      header: "Individual Users",
      cell: ({ row }) => `${row.original.selectedUserIds.length} users selected`
    }
  ];
    ...
    <div className="p-6">
      <div className="bg-white p-6 rounded-xl shadow mb-10">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div className="flex gap-4">
            <div className="flex-1">
              <label className="block text-sm font-medium mb-1">Select Audience Group (Optional)</label>
              <select
                className="w-full border px-3 py-2 rounded-md focus:ring-blue-500 focus:border-blue-500"
      ...
      <h2 className="text-2xl font-bold text-gray-800 mb-3">Notification History 📜</h2>
